...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${scrolled ? 'bg-white/70 dark:bg-dark-bg/80 backdrop-blur-lg border-slate-200/50 dark:border-dark-border shadow-sm' : 'bg-transparent border-transparent'}`}>
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden bg-white/95 dark:bg-dark-bg/95 backdrop-blur-lg border-t border-brand-100 dark:border-dark-border px-4 py-4 space-y-2 animate-fade-in">
          {navLinks.map((item) => (
            <Link 
              key={item.label} 
              to={item.to}
